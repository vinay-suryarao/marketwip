  ...
  return (
    <main className="mx-auto flex w-full max-w-[1200px] flex-1 flex-col gap-6 px-6 py-10 md:px-8">
      <section className="rounded-3xl border border-[#1A2552] bg-[#0A102E] p-7 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] relative overflow-hidden">
        ...
        <h1 className="text-4xl font-extrabold font-display text-transparent bg-clip-text bg-gradient-to-r from-white to-[#00F0FF] relative z-10">Market Video Signals</h1>
      ...
      {isLoading ? (
        ...
        <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {videos.map((video) => (
            <article key={video.id} className="rounded-2xl border border-[#1A2552] bg-[#0A102E] p-5 shadow-lg relative overflow-hidden group">
              ...
              <h2 className="mt-2 text-xl font-bold font-display text-white relative z-10">{video.tag}</h2>
