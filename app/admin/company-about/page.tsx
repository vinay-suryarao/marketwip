...
import { getCustomCompanyAbout, saveCustomCompanyAbout } from "@/lib/services/companyAboutService";
...
export default function CompanyAboutAdminPage() {
  ...
  const handleFetch = async () => {
    ...
    try {
      const custom = await getCustomCompanyAbout(symbol.trim());
      if (custom) {
        setAboutText(custom);
        setIsCustom(true);
        showPrompt("Loaded custom override from database.", "success");
      } else {
        const res = await fetch(`/api/analyzer/stock?name=${encodeURIComponent(symbol.trim())}`);
        if (res.ok) {
          const apiData = await res.json();
          let apiText = "";
          if (typeof apiData.companyProfile === "string") {
            apiText = apiData.companyProfile;
          } else if (apiData.companyProfile && typeof apiData.companyProfile.companyDescription === "string") {
            apiText = apiData.companyProfile.companyDescription;
          }
          setAboutText(apiText);
          setIsCustom(false);
          showPrompt("Loaded default info from Indian API.", "success");
        } else {
          showPrompt("Failed to fetch company info", "error");
          setAboutText("");
          setIsCustom(false);
        }
      }
  ...
  const handleSave = async () => {
    if (!symbol.trim() || !aboutText.trim()) {
      showPrompt("Symbol and About Text are required", "error");
      return;
    }
    ...
    try {
      await saveCustomCompanyAbout(symbol.trim(), aboutText.trim());
      showPrompt("Custom info saved successfully!", "success");
      setIsCustom(true);
    } catch (err) {
      showPrompt(err instanceof Error ? err.message : "Failed to save custom info", "error");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-[#6074a0]">
              Override the default Indian API "About" text for any company.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-6 max-w-3xl">
            ...
            <div className="flex flex-col gap-2 mt-4">
              ...
              <textarea
                ...
                placeholder="Fetch a symbol first or start typing the custom text..."
                rows={12}
            ...
            <div className="mt-4 flex justify-end">
              <button
                onClick={handleSave}
                disabled={isLoading || !symbol || !aboutText}
